...
[
   ...
   AC_CACHE_VAL(ac_cv_path_moc,
   [
     ac_cv_path_moc="NONE"
     if test -n "$MOC"; then
        ac_cv_path_moc="$MOC";
     else
        for mocdir in $ac_qt_bindir $QTDIR/bin \
	    /usr/bin /usr/X11R6/bin /usr/lib/qt/bin \
	    /usr/local/qt/bin; do

	  if test -f "$mocdir/moc"; then
            ac_cv_path_moc="$mocdir/moc"
            break
          fi
	done

	if test "$ac_cv_path_moc" = "NONE"; then
	    kde_save_IFS=$IFS
	    IFS=':'
	    for mocdir in $PATH; do
		if test -f "$mocdir/moc"; then
		    ac_cv_path_moc="$mocdir/moc"
		    break
		fi
...
if test "$ac_qt_includes" = NO || test "$ac_qt_libraries" = NO; then
  ...
  AC_MSG_ERROR([Qt-1.41 $ac_qt_notfound not found. Please check your installation! ]);
else
  have_qt="yes"
fi
])
...
[
   ...
   AC_LANG_C
   kde_save_LIBS="$LIBS"
   LIBS="$LIBS $all_libraries $LIBJPEG $LIBTIFF $LIBPNG -lqt -lX11 $LIBSOCKET -lm"
   AC_CHECK_LIB(kimgio, main, [
